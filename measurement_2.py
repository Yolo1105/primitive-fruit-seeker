...
def extract_and_group_json(input_file, output_file):
    ...
    start_marker = 'class="pip-product-dimensions__dimensions-container"'
    end_marker = '</p> </div>'

    start_index = body_content.find(start_marker)
    end_index = body_content.find(end_marker, start_index)
    ...
    extracted_content = body_content[start_index:end_index + len(end_marker)]
